...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' 
  ...
  # <--- This is likely the misplaced brace causing the error
...
def encode_heatmap_to_base64(heatmap_array: np.array) -> str:
    """
    Converts a 2D numpy attention map array into a base64 encoded PNG string
    suitable for direct embedding in HTML/CSS (e.g., as a background image).
    """
    # Scale map (0-1) to 8-bit grayscale (0-255)
    img_data = (heatmap_array * 255).astype(np.uint8)
    
    # Create PIL Image from grayscale array
    img = Image.fromarray(img_data, mode='L') # 'L' mode is 8-bit pixels, black and white
...
def predict():
    """Handles image upload, runs ML prediction, and returns results."""
    ...
    if file:
        ...
        try:
            ...
            # 2. Run Inference
            # The 'predict_leaf' function handles the TFLite model execution (mocked here)
            results = predict_leaf(temp_path)
            
            # 3. Process Attention Map for Frontend
            heatmap_b64 = encode_heatmap_to_base64(results['attention_map'])
            
            # 4. Prepare final response payload
            response_data = {
                'status': 'success',
                ...
                'heatmap_overlay': heatmap_b64 # Base64 string for image display
            }
...
def chat():
    ...
    try:
        # Use the retrieval-based module
